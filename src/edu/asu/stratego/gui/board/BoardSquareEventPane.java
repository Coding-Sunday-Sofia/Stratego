...
public class BoardSquareEventPane extends GridPane {
	...
	/**
	 * This event is fired when the cursor leaves the square. It changes the hover
	 * image back to its default image: a blank image with a 1% fill.
	 */
		...
		public void handle(MouseEvent e) {
			...
			if (Game.getStatus() == GameStatus.SETTING_UP)
				...
				if (Game.getMoveStatus() == MoveStatus.OPP_TURN)
					...
					// Moved elsewhere: Function to only allow highlighting of squares piece can
					// move to
				}
			}
		};
	...
	private void displayValidMoves(int pieceRow, int pieceCol) {
		...
		for (Point validMove : validMoves) {
			Game.getBoard().getSquare((int) validMove.getX(), (int) validMove.getY()).getEventPane().getHover()
	...
	private ArrayList<Point> computeValidMoves(int row, int col) {
		...
		// Set the max distance of a valid move to the board width if the piece is a
		// scout
		...
		// 3) If the square has no piece on it OR there is a piece, but it is an
		// opponent piece
		...
		if (pieceType != PieceType.BOMB && pieceType != PieceType.FLAG) {
			...
			for (int i = -1; i >= -max; --i) {
				if (isInBounds(row + i, col)
						&& (!isLake(row + i, col) || (!isNullPiece(row + i, col) && !isOpponentPiece(row + i, col)))) {
			...
			for (int i = 1; i <= max; ++i) {
				if (isInBounds(row, col + i)
						&& (!isLake(row, col + i) || (!isNullPiece(row, col + i) && !isOpponentPiece(row, col + i)))) {
	...
	/**
	 * During the Setup phase of the game, this method randomly places the pieces
	 * that have not yet been placed when the Setup Timer hits 0.
	 */
		...
		for (int col = 0; col < 10; ++col) {
			for (int row = 6; row < 10; ++row) {
				...
				if (squarePiece == null) {
					...
					// While the pieceType that is going to be placed is null, loop finding a random
					// one
					...
					while (pieceType == null) {
						...
						// There are no more available for that piecetype, remove it from the array so
						// it won't be randomly generated again
						else
